...
> Ported from the Claude Code subagent `linkedin-lab-dr`.
> >
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py search "sales page structure" --course Medhora --limit 10
```
...
`linkedin-lab-psych`; register and authenticity to `linkedin-lab-voice`.
