...
> Ported from the Claude Code subagent `linkedin-lab-psych`.
> >
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py search "social proof" --course Schutz --limit 10
```
...
`linkedin-lab-dr` and `linkedin-lab-voice`. Note anything glaring in one line and move
on.
