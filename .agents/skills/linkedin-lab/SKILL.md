...
name: linkedin-lab
description: >
  Route explicit or multi-step linkedin-lab requests across writing, critique,
  ...
  "linkedin-lab", asks which linkedin-lab workflow to use, or combines two or more
  operations such as critique then rewrite. For a single clear operation, use
  linkedin-lab-write, linkedin-lab-critique, linkedin-lab-swipe, linkedin-lab-mine, or
  linkedin-lab-vault directly.
---
...
| Request | Sub-skill |
|---------|-----------|
| Write new copy of any kind | `linkedin-lab-write` |
| Rewrite supplied copy to sound natural or less generic | `linkedin-lab-write` |
| Score, audit, or critique existing copy | `linkedin-lab-critique` |
| "What does {instructor} say about {topic}?" | `linkedin-lab-swipe` |
| Re-mine the corpus, ingest books, refresh references | `linkedin-lab-mine` |
| OCR a scanned copywriting PDF | `linkedin-lab-mine` |
| Build, refresh, or validate the Obsidian vault | `linkedin-lab-vault` |
...
Resolve `scripts/linkedin_lab.py` from this skill directory and invoke it by its
absolute path. Deterministic retrieval and maintenance belong in the CLI.

```bash
python scripts/linkedin_lab.py doctor
python scripts/linkedin_lab.py search "social proof" --source all --limit 15
python scripts/linkedin_lab.py ingest
python scripts/linkedin_lab.py ocr SOURCE.pdf --out-dir /external/ocr/source-id
python scripts/linkedin_lab.py transcribe SOURCE.m4b --model /local/faster-whisper-model --out-dir /external/transcription/source-id
```

```bash
python scripts/linkedin_lab.py extract --find "AIDA formula" --from 04:00 --to 08:00
```

| Script | Purpose |
|--------|---------|
| `linkedin_lab.py` | Unified entry point for all commands |
...
Run `linkedin_lab.py config set corpus PATH` once for a global installation. The
resolver also accepts `--corpus`, `linkedin_lab_CORPUS`, and a `_transcripts`
directory in the current workspace. Mining writes to an external
`.linkedin-lab-data` directory, never into the installed skill. Original sources
...
- If the book index is absent, continue with course sources and say books were
  not searched; `linkedin_lab.py doctor` shows the missing path.
...
- If source validation reports drift, do not cite the stale local index. Re-run
  `linkedin_lab.py ingest`, then search again.
...
LinkedIn Lab verified that primary source.
