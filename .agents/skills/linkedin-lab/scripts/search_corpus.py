...
from linkedin_lab_config import resolve_corpus, resolve_mined_store, resolve_source_store
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.source in {"books", "all"}:
        ...
        if book_store.is_file():
            ...
            print(
                f"error: local source store not found: {book_store}; run linkedin_lab.py ingest",
                file=sys.stderr,
            )
            return 2
