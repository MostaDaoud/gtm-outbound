...
Only generated passages and hashes are stored in ``.linkedin-lab-data``. Original
...
def validate_source_index(
    ...
    sources_list = sources_data.values() if isinstance(sources_data, dict) else sources_data
    for source in sources_list:
        path_str = source.get("path") or source.get("source_path", "")
        if not path_str:
            continue
        path = Path(path_str)
        if not path.is_file():
            findings.append({"severity": "High", "file": str(path), "issue": "Source file is missing"})
        elif file_hash(path) != (source.get("sha256") or source.get("source_sha256")):
        ...
        if not source.get("records") and not source.get("passages") and source.get("status") != "needs_ocr":
