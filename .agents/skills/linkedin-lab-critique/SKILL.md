...
description: >
  ...
  email", "why isn't this converting", "roast my copy", "linkedin-lab critique", or
  ...
  copy for assessment. For writing or fully rewriting copy, use linkedin-lab-write
  instead.
---

# LinkedIn Lab — Critique
...
collections with `linkedin_lab.py search "topic" --source all`. Cite course material
...
Load `../linkedin-lab/references/critique-rubric.md`. It carries the five categories, their
...
`../linkedin-lab/references/fascination-bullets.md`. Apply its block-level checks
...
close, load `../linkedin-lab/references/classic-direct-response.md`. For welcome
sequences, consulting outreach, sales email QA, or formula-driven headlines,
load `../linkedin-lab/references/lead-and-email-playbooks.md`. Keep their findings
inside the existing rubric and enforce LinkedIn Lab's proof and scarcity rules over
any source template.

If the user asks whether the copy sounds AI-like, generic, over-polished, or
unlike their voice, also load `../linkedin-lab/references/anti-generic-copy.md`.
...
`../linkedin-lab/references/harry-dry-copywriting.md`. Use its visual,
...
messaging architecture, also load `../linkedin-lab/references/positioning.md`. Check
...
the three files in `../linkedin-lab/references/lenses/` and merge their findings:
...
Use the format in `../linkedin-lab/references/critique-rubric.md`. Two requirements:
...
the findings, hand off to `linkedin-lab-write` with the findings as input.
