...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="copy-lab-vault-test-") as tmp:
        base = Path(tmp)
        copy_lab = base / "copy-lab"
        data = base / ".copy-lab-data"
        vault = base / "vault"
        ...
        reference_dir = copy_lab / "references"
        ...
        write_json(
            copy_lab / "assets" / "corpus_index.json",
            {
        ...
        write_json(
            copy_lab / "assets" / "library_manifest.json",
            {"references": manifest_references},
        )
        write_json(
            copy_lab / "assets" / "mining" / "mining_report.json",
        ...
        command = [str(BUILD), "--copy-lab-dir", str(copy_lab), "--data-dir", str(data), "--vault", str(vault)]
        first = run(*command)
        assert first.returncode == 0, first.stderr
        manifest_one = (vault / "_meta" / "build-manifest.json").read_bytes()
        assert (vault / "Copy Lab Dashboard.md").is_file()
    ...
    print("Copy Lab vault tests passed")
    return 0
