---
name: linkedin-lab-mine
description: >
  Re-mine the copywriting transcript corpus, OCR scanned PDFs, transcribe long-form audio,
  ingest local books and captioned media, and refresh the linkedin-lab reference library. Rebuilds course indexes,
  ...
  corpus", "refresh the library", "add a course to linkedin-lab", "rebuild the
  index", "update the references", "OCR this PDF", "process these scanned
  letters", "linkedin-lab mine", or when validate_library reports the references
  are stale or thin. This is the maintenance path for linkedin-lab, not a writing
...
**Shared resources.** Resolve the sibling `../linkedin-lab/` directory from this
`SKILL.md` directory, then use the absolute path to
`scripts/linkedin_lab.py`. Mutable indexes, mining output, and configuration live
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py config set course_map /absolute/path/to/course_map.json
```
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py index
```
...
Keep the catalog outside the skill at `.linkedin-lab-data/sources/catalog.json`.
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py ingest
```
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py ocr /absolute/source.pdf \
  --out-dir /absolute/.linkedin-lab-data/ocr/source-id --jobs 4
```
...
```bash
python -m pip install -r /absolute/path/to/linkedin-lab/requirements-transcription.txt
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py transcribe /absolute/source.m4b \
  --model /absolute/local-faster-whisper-model \
  --out-dir /absolute/.linkedin-lab-data/transcription/source-id
```
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py mine --per-theme 75 --min-score 7
```
...
```bash
python /absolute/path/to/linkedin-lab/scripts/linkedin_lab.py validate
```
...
**Corpus not found.** Run `linkedin_lab.py config set corpus /absolute/path/to/_transcripts`,
set `linkedin_lab_CORPUS` for a one-off override, or run from a workspace whose
ancestor contains `_transcripts/`. Use `linkedin_lab.py doctor` to see the active
configuration.
...
**Search returns nothing after re-mining.** `linkedin_lab.py search` reads the
...
text can be selected or extracted, then rerun `linkedin_lab.py ingest` with a Python
