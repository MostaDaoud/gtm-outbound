#!/usr/bin/env python3
"""Build a provenance-aware Obsidian vault from LinkedIn Lab artifacts."""
...
def topic_notes(writer: VaultWriter, references: Path, library: dict[str, Any]) -> dict[str, str]:
    ...
    for source in sorted(references.glob("*.md"), key=lambda p: p.name.casefold()):
        ...
        note += f"# {title}\n\n"
        note += "> [!info] Rebuildable LinkedIn Lab reference\n"
        note += "> This note mirrors the distilled reference library. Put personal annotations in `Source Notes/`.\n\n"
        note += "[[LinkedIn Lab Dashboard|Dashboard]] · [[Topic Library]]\n\n---\n\n" + body
...
def course_notes(writer: VaultWriter, corpus: dict[str, Any], topics: dict[str, str]) -> list[str]:
    ...
    for course in corpus.get("courses", []):
        ...
        note += f"# {name}\n\n[[LinkedIn Lab Dashboard|Dashboard]] · [[Course Library]]\n\n"
        note += "## Coverage\n\n"
        ...
        note += "\n## Search this course\n\n"
        note += f"```powershell\npython <linkedin-lab>/scripts/linkedin_lab.py search \"QUERY\" --course {json.dumps(name, ensure_ascii=False)} --limit 15\n```\n"
        note += "\n> [!note]\n> Use LinkedIn Lab search for quotations and timestamps; this note is a map, not the transcript.\n"
        writer.generated_file(relative, note)
    return links
...
def source_notes(
    ...
    for item in catalog.get("sources", []):
        ...
        note += "\n## Retrieve passages\n\n"
        note += f"```powershell\npython <linkedin-lab>/scripts/linkedin_lab.py search \"QUERY\" --source books --course {json.dumps(author, ensure_ascii=False)} --limit 15\n```\n"
...
def theme_notes(writer: VaultWriter, mining: dict[str, Any], topics: dict[str, str]) -> list[str]:
    ...
    for key, stats in sorted(mining.get("themes", {}).items()):
        ...
        note += "```powershell\npython <linkedin-lab>/scripts/linkedin_lab.py search \"QUERY\" --source all --limit 15\n```\n"
        writer.generated_file(relative, note)
    return links


def index_note(title: str, note_type: str, rows: list[list[object]], headers: list[str]) -> str:
    note = frontmatter(title, note_type, ["linkedin-lab/dashboard"])
    note += f"# {title}\n\n[[LinkedIn Lab Dashboard|Dashboard]]\n\n"
    note += "| " + " | ".join(headers) + " |\n"
...
def build_dashboards(
    ...
    dashboard = frontmatter("LinkedIn Lab Dashboard", "linkedin-lab-dashboard", ["linkedin-lab/dashboard"])
    dashboard += "# LinkedIn Lab Dashboard\n\n"
    dashboard += "> [!abstract] LinkedIn Lab at a glance\n"
    ...
    dashboard += "- [[LinkedIn Lab Workflow.canvas|Workflow canvas]]\n\n"
    ...
    writer.generated_file("LinkedIn Lab Dashboard.md", dashboard)
    ...
    writer.generated_file("Dashboards/Course Library.md", index_note("Course Library", "linkedin-lab-course-index", course_rows, ["Course", "Lessons", "Words", "Hours"]))

    topic_rows = [[f"[[{link}|{Path(link).name}]]", source] for source, link in sorted(topics.items())]
    writer.generated_file("Dashboards/Topic Library.md", index_note("Topic Library", "linkedin-lab-topic-index", topic_rows, ["Topic", "Reference file"]))

    source_rows = [[f"[[{link}|{Path(link).name}]]"] for link in source_links] or [["Local source catalog not available"]]
    writer.generated_file("Dashboards/Source Library.md", index_note("Source Library", "linkedin-lab-source-index", source_rows, ["Indexed source"]))

    theme_rows = [[f"[[{link}|{Path(link).name}]]"] for link in theme_links]
    writer.generated_file("Dashboards/Theme Coverage.md", index_note("Theme Coverage", "linkedin-lab-theme-index", theme_rows, ["Mining theme"]))

    health = frontmatter("Library Health", "linkedin-lab-health", ["linkedin-lab/dashboard"])
    ...
    health += "> [!info]\n> Run the vault validator after every LinkedIn Lab re-mine. These totals describe indexed material, not independent factual verification.\n"
    ...
    writer.generated_json("Dashboards/LinkedIn Lab Workflow.canvas", canvas)
...
Open [[LinkedIn Lab Dashboard]] first.

This vault has two kinds of material:

- `_generated/` and `Dashboards/` are rebuilt from LinkedIn Lab indexes and references.
- `Projects/`, `Inbox/`, `Source Notes/`, `Templates/`, and `Workflows/` belong to you.

Use the core Templates plugin to create a brief or project note. Use LinkedIn Lab's CLI when you need a source passage, timestamp, page, or chapter; Obsidian is the operating surface, while LinkedIn Lab search remains the retrieval authority.
""",
...
5. Search the corpus for missing evidence or examples with `linkedin_lab.py search`.
...
2. Run `python <linkedin-lab>/scripts/linkedin_lab.py search \"QUERY\" --source all --limit 15`.
...
1. Run `linkedin_lab.py doctor`.
2. If courses changed, run `linkedin_lab.py index` and `linkedin_lab.py mine`.
3. If local sources changed, run `linkedin_lab.py ingest`.
4. Validate and update distilled references in a staged LinkedIn Lab package.
5. Run the `linkedin-lab-vault` builder.
...
def seed_obsidian(writer: VaultWriter) -> None:
    ...
    writer.seed_json(".obsidian/appearance.json", {"baseFontSize": 16, "enabledCssSnippets": ["linkedin-lab"]})
    writer.seed_json(
    ...
    writer.seed_json(
        ".obsidian/graph.json",
        {
            ...
            "showOrphans": False,
            "colorGroups": [
                {"query": "tag:#linkedin-lab/course", "color": {"a": 1, "rgb": 3447003}},
                {"query": "tag:#linkedin-lab/topic", "color": {"a": 1, "rgb": 16750848}},
                {"query": "tag:#linkedin-lab/source", "color": {"a": 1, "rgb": 10181046}},
                {"query": "tag:#linkedin-lab/project", "color": {"a": 1, "rgb": 15158332}},
            ],
        },
    )
    writer.seed_file(
        ".obsidian/snippets/linkedin-lab.css",
        """body { --linkedin-lab-accent: #f28c28; }
.theme-dark { --linkedin-lab-accent: #ffad55; }
...
.tag[href="#linkedin-lab/project"] { background: color-mix(in srgb, var(--linkedin-lab-accent) 24%, transparent); }
""",
    )
...
def clean_stale(root: Path, prior: dict[str, Any], current: set[str]) -> list[str]:
    ...
    if not isinstance(prior_files, dict):
        return removed
    allowed_prefixes = ("_generated/", "Dashboards/", "LinkedIn Lab Dashboard.md")
...
def build(linkedin_lab: Path, data_dir: Path, vault: Path) -> dict[str, Any]:
    assets = linkedin_lab / "assets"
    references = linkedin_lab / "references"
    required = [
    ...
    if missing:
        raise FileNotFoundError("missing required LinkedIn Lab input(s): " + ", ".join(missing))
    ...
    manifest = {
        "schema_version": 1,
        "build_id": build_id,
        "linkedin_lab_dir": str(linkedin_lab.resolve()),
    ...
    config = {"linkedin_lab_dir": str(linkedin_lab.resolve()), "data_dir": str(data_dir.resolve()), "vault": str(vault.resolve())}
...
def default_paths() -> tuple[Path, Path, Path]:
    ...
    return skill.parent / "linkedin-lab", workspace / ".linkedin-lab-data", workspace / "LinkedIn Lab Vault"


def main(argv: list[str] | None = None) -> int:
    default_linkedin_lab, default_data, default_vault = default_paths()
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--linkedin-lab-dir", type=Path, default=default_linkedin_lab)
    parser.add_argument("--data-dir", type=Path, default=default_data)
    parser.add_argument("--vault", type=Path, default=default_vault)
    args = parser.parse_args(argv)
    try:
        result = build(args.linkedin_lab_dir, args.data_dir, args.vault)
